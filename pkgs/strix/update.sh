#!/usr/bin/env bash
### Prints {"version":"...","url":"..."} for the latest macos-arm64 release,
...
release="$(curl -fsSL https://api.github.com/repos/usestrix/strix/releases/latest)"

version="$(jq -r '.tag_name | ltrimstr("v")' <<<"$release")"
url="$(jq -r '.assets[] | select(.name | test("^strix-.*-macos-arm64\\.tar\\.gz$")) | .browser_download_url' <<<"$release")"

if [[ -z "$version" || "$version" == "null" || -z "$url" || "$url" == "null" ]]; then
  echo "error: couldn't find version/macos-arm64 asset in the latest usestrix/strix release" >&2
  exit 1
fi
